...
def learning_curves_plot(train_sizes, mean_acc, std_acc, exp, curve_type):
    ...
    plt.figure( dpi=80)
    # Draw lines
    plt.fill_between(train_sizes, mean_acc - std_acc, mean_acc + std_acc, alpha=0.1,color="r")
    plt.plot(mean_acc, '-', color="r",  label="ResNet")


    plt.fill_between(train_sizes, mean_list[0]- std_list[0],  mean_list[0] + std_list[0], alpha=0.1,color="darkgreen")
    plt.plot(mean_list[0], '-', color="darkgreen",  label="ResNet-0.25 AEs")

    plt.fill_between(train_sizes,  mean_list[1] - std_list[1],  mean_list[1] + std_list[1], alpha=0.1,color="darkblue")
