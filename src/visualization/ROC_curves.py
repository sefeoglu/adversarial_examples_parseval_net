...
def plot_roc(fpr_l,tpr_l,roc_auc_l,std_tpr_list, std_roc_auc_list, epsilon, fig_name):

  plt.figure(figsize=(10, 8))
  lw = 2

  plt.fill_between(fpr_l, tpr_l[0]- std_tpr_list[0],  tpr_l[0] + std_tpr_list[0], alpha=0.1,color="purple")
  plt.plot(fpr_l, tpr_l[0], color='purple',
          lw=lw, label='ResNet (area = %0.4f)' % roc_auc_l[0])
  
  plt.fill_between(fpr_l, tpr_l[1]- std_tpr_list[1],  tpr_l[1] + std_tpr_list[1], alpha=0.1,color="darkgreen")
  plt.plot(fpr_l, tpr_l[1], color='darkgreen',
          lw=lw, label='ResNet-0.25 AEs(area = %0.4f)' % roc_auc_l[1])
  
  plt.fill_between(fpr_l, tpr_l[2]- std_tpr_list[2],  tpr_l[2] + std_tpr_list[2], alpha=0.1,color="darkblue")
  plt.plot(fpr_l, tpr_l[2], color='darkblue',
           lw=lw, label='ResNet-0.50 AEs(area = %0.4f)' % roc_auc_l[2])
  
  plt.fill_between(fpr_l, tpr_l[3]- std_tpr_list[3],  tpr_l[3] + std_tpr_list[3], alpha=0.1,color="pink")
  plt.plot(fpr_l, tpr_l[3], color='pink',
          lw=lw, label='ResNet-0.75 AEs(area = %0.4f)' % roc_auc_l[3])
  
  plt.fill_between(fpr_l, tpr_l[4]- std_tpr_list[4],  tpr_l[4] + std_tpr_list[4], alpha=0.1,color="darkorange")
  plt.plot(fpr_l, tpr_l[4], color='darkorange',
          lw=lw, label='ResNet-1.0 AEs (area = %0.4f)' % roc_auc_l[4])
  
  plt.fill_between(fpr_l, tpr_l[5]- std_tpr_list[5],  tpr_l[5] + std_tpr_list[5], alpha=0.1,color="darkmagenta")
  plt.plot(fpr_l, tpr_l[5], color='darkmagenta',
          lw=lw, label='Parseval (area = %0.4f)' % roc_auc_l[5])
...
for exp in Experiment:
    ...
    for epsilon  in epsilons_list:
        ...
        for i in range(10):
            ...
            if percent != 0:
            model_path = prefix + ex+"/ResNet_"+str(epsilon)+"_"+str(percent)+"_"+str(i)+".h5" 
            print(model_path)
        ...
        plot_roc(mean_fpr,mean_tpr_list,mean_roc_auc_list, std_tpr_list, std_roc_auc_list, epsilon, fig_name)